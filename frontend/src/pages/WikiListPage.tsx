...
import { Plus, BookOpen, Search, Lock, X, Clock, User, ChevronRight } from "lucide-react";
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-4 page-enter">
      ...
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-[20px] font-semibold tracking-tight text-[var(--text)]">{t("wiki.title", "知识库")}</h1>
      ...
      <div className="card rounded-[8px] px-4 py-3 space-y-3">
        <div className="relative">
          <Search size={14} strokeWidth={1.8} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-[var(--text-faint)]" />
          <input
            type="text"
            value={searchInput}
            ...
            className="w-full rounded-[6px] border border-[var(--border)] bg-[var(--bg)] py-[8px] pl-9 pr-9 text-[13px] text-[var(--text)] outline-none transition-all placeholder:text-[var(--text-faint)] hover:border-[#d1d5db] focus:border-[var(--primary)] focus:bg-white focus:shadow-[0_0_0_3px_rgba(37,99,235,.08)]"
          />
          {searchInput && (
        ...
        <div className="flex items-center gap-1">
          ...
          {search && (
            <span className="ml-2 flex items-center gap-1 text-[11px] text-[var(--text-faint)]">
              搜索: "{search}"
              <button onClick={() => { setSearch(""); setSearchInput(""); setPage(1); }} className="text-[var(--text-muted)] hover:text-red-500 transition-colors">×</button>
            </span>
          )}
        </div>
      </div>

      {/* Page List — each item as a flat card */}
      {pages.length === 0 ? (
        <div className="card flex flex-col items-center justify-center py-16 rounded-[8px] text-[var(--text-muted)]">
          <BookOpen size={32} className="mb-2 opacity-15" strokeWidth={1.5} />
          <p className="text-[13px] font-medium text-[var(--text-secondary)]">{t("wiki.no_pages", "没有页面")}</p>
          ...
          {pages.map(page => {
            ...
            return (
              <div
                ...
                className="card flex items-start gap-3.5 rounded-[8px] px-4 py-3.5 cursor-pointer transition-all hover:border-[#d1d5db] group"
              >
                <BookOpen size={17} strokeWidth={1.8} className="mt-0.5 text-[var(--text-faint)] shrink-0 group-hover:text-[var(--primary)] transition-colors" />
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    {!page.is_public && (
                      <Lock size={11} className="text-[var(--text-faint)] shrink-0" strokeWidth={1.8} />
                    )}
                  </div>
                  {hasPreview && (
                      ...
                      <>
                        ...
                        <span className="flex items-center gap-1">
                          {page.tags.split(",").filter(t => t.trim()).slice(0, 3).map(tag => (
                            <span key={tag} className="rounded-[4px] bg-[var(--bg-muted)] px-1.5 py-0.5 text-[10px] text-[var(--text-muted)]">
                              {tag.trim()}
                            </span>
                          ))}
                        </span>
                ...
                <ChevronRight size={15} strokeWidth={1.8} className="mt-1 text-[var(--text-faint)] shrink-0 group-hover:text-[var(--primary)] group-hover:translate-x-0.5 transition-all" />
              </div>
            );
          })}
        </div>
