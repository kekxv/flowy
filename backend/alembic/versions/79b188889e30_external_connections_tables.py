...
down_revision: str | None = "2a561e160fc0"
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "direction IN ('import','export','bidirectional')", name="ck_sync_direction"
        ),
        sa.CheckConstraint(
            "status IN ('running','success','partial','failed')", name="ck_sync_status"
        ),
        sa.ForeignKeyConstraint(["connection_id"], ["external_connections.id"], ondelete="CASCADE"),
        sa.PrimaryKeyConstraint("id"),
    )
