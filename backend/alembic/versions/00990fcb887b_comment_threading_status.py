...
down_revision: Union[str, None] = 'm001'
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    with op.batch_alter_table('issue_comments') as batch_op:
        ...
        batch_op.add_column(sa.Column('status', sa.String(length=16), nullable=False, server_default='valid'))
        batch_op.add_column(sa.Column('status_changed_by', sa.String(length=36), nullable=True))
        batch_op.create_foreign_key('fk_comment_parent', 'issue_comments', ['parent_id'], ['id'], ondelete='CASCADE')
        batch_op.create_foreign_key('fk_comment_status_changer', 'users', ['status_changed_by'], ['id'])
