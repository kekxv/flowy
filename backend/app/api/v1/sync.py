...
async def get_sync_logs(
    ...
    result = await db.execute(
        select(SyncLog).where(SyncLog.connection_id == connection_id).order_by(SyncLog.started_at.desc()).limit(10)
    )
    logs = result.scalars().all()
    return [
        {
            "id": l.id, "status": l.status, "direction": l.direction,
            "issues_processed": l.issues_processed, "issues_created": l.issues_created,
            "issues_updated": l.issues_updated, "errors": l.errors,
