...
- `/comment <id> <内容>` — 添加评论
- `/comment <id> 追加 <内容>` — 将内容追加到问题描述（不创建评论；可引用消息后发送）

#### 查询命令
- `/list [all]` — 列出问题（默认显示我的，all 显示全部）
- `/stats` — 问题统计（按状态/优先级分组，显示进度条）
- `/soft [关键词]` — 软件版本说明与下载信息（无参数列出全部组件最新版本，带关键词查看指定组件版本历史）
...
/comment #12345678 追加 补充复现步骤：切换到英文后重试
```

引用消息后执行 `/create` 会将引用内容一并写入描述；执行
`/comment <id> 追加` 则会把命令文本和引用内容都追加到该问题的描述详情中，而不是新增评论。
